...
if (strpos($contentType, 'application/json') !== false) {
    ...
    if ($input) {
        ...
        $name = $email = $phone = $country = $message = $sourcePage = $selectedPlan = '';
    }
} else {
    // Form data
    $name         = $_POST['name'] ?? '';
...
debugLog("Extracted values - Name: '$name', Email: '$email', Phone: '$phone', Country: '$country', Message: '$message', SourcePage: '$sourcePage', SelectedPlan: '$selectedPlan'");
...
$stmt->bind_param("sssssss", $name, $email, $phone, $country, $message, $sourcePage, $selectedPlan);
...
if ($stmt->execute()) {
    ...
    // Build Selected Plan row only if it exists
    $selectedPlanRow = '';
    if (!empty($selectedPlan)) {
        $selectedPlanRow = '<tr><td class="label" style="background:#e3f2fd;">Selected Plan</td><td class="value" style="background:#e3f2fd; color:#0066cc; font-weight:600;">' . htmlspecialchars($selectedPlan) . '</td></tr>';
    }
