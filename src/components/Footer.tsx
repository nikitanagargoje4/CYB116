    ...
    <div className="container max-w-7xl mx-auto mobile-padding">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-6 md:mb-8">
        ...
        <div>
          ...
          <ul className="space-y-2">
            {["cloud-solutions", "enterprise-solutions", "cybersecurity-services", "ai-data-analytics", "digital-marketing", "managed-services"].map((path) => (
              <li key={path}>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {["about", "leadership", "careers", "news", "support"].map((path) => (
              <li key={path}>
                <a href={`/${path}`} className="text-white/80 hover:text-accent text-sm transition-colors block py-0.5">
                  {path.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
                </a>
            ...
            <li>
              <a href="/resources" className="text-white/80 hover:text-accent text-sm transition-colors block py-0.5">
                Case Studies
              </a>
            </li>
          </ul>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 text-center md:text-left">
        ...
        <div className="flex flex-wrap justify-center md:justify-end gap-4 text-sm">
          <a href="/privacy-policy" className="text-white/60 hover:text-accent transition-colors touch-target">
            Privacy Policy
          </a>
          <a href="/privacy-policy#third-party" className="text-white/60 hover:text-accent transition-colors touch-target">
            Terms of Service
          </a>
          <a href="/privacy-policy#cookies" className="text-white/60 hover:text-accent transition-colors touch-target">
            Cookie Policy
          </a>
        </div>
      </div>
